    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="px-4 pt-6">
        <div className="bg-gradient-to-br from-[#E6F9EE] to-[#F0FFF4] rounded-2xl p-8 text-center">
          ...
          <h2 className="text-xl font-black text-gray-900 mb-2">الكروت المشتراة</h2>
          <p className="text-gray-500 text-sm mb-4">يجب تسجيل الدخول أولاً</p>
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ type: "spring", stiffness: 120, damping: 14 }} className="px-4 pt-4">
      <div className="bg-gradient-to-br from-[#E6F9EE] to-[#F0FFF4] rounded-2xl p-4 mb-4">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-xl font-black text-gray-900 flex items-center gap-2"><ShoppingBag className="w-6 h-6 text-[#1B7A3D]" />الكروت المشتراة</h2>
          <Badge className="bg-[#E8F5E9] text-[#1B7A3D]">{purchasedCards.length} كرت</Badge>
        </div>
        <p className="text-sm text-gray-500">جميع الكروت التي قمت بشرائها</p>
      </div>
      ...
      {purchasedCards.length === 0 ? (
        <div className="bg-white rounded-2xl card-shadow p-8 text-center mt-4">
          ...
          <h3 className="text-lg font-bold text-gray-900 mb-2">لا توجد كروت مشتراة</h3>
          <p className="text-gray-500 text-sm">لم تقم بشراء أي كرت بعد</p>
        </div>
      ) : (
          ...
          {purchasedCards.map((card, i) => (
            <motion.div key={card.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05, type: "spring", stiffness: 200, damping: 20 }} className="bg-white rounded-2xl card-shadow overflow-hidden">
              <div className="bg-gradient-to-l from-[#1B7A3D] to-[#22A24D] px-4 py-2.5 flex items-center justify-between">
                <div className="flex items-center gap-2"><Check className="w-4 h-4 text-white" /><span className="text-white font-bold text-sm">تم الشراء بنجاح</span></div>
              ...
              <div className="p-4">
                <div className="bg-[#E8F5E9] rounded-xl p-3 mb-3">
                  <p className="text-[10px] text-gray-500 font-bold mb-1">رمز الكرت</p>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-gray-50 rounded-xl p-2 text-center"><HardDrive className="w-4 h-4 text-[#1B7A3D] mx-auto mb-1" /><p className="text-[10px] text-gray-400">البيانات</p><p className="text-xs font-bold text-gray-900">{card.cardData || "-"}</p></div>
                  <div className="bg-gray-50 rounded-xl p-2 text-center"><Clock className="w-4 h-4 text-[#1B7A3D] mx-auto mb-1" /><p className="text-[10px] text-gray-400">المدة</p><p className="text-xs font-bold text-gray-900">{card.cardDuration ? (card.cardDuration === 2 ? "يومان" : `${card.cardDuration} أيام`) : "-"}</p></div>
                  <div className="bg-gray-50 rounded-xl p-2 text-center"><Wallet className="w-4 h-4 text-[#1B7A3D] mx-auto mb-1" /><p className="text-[10px] text-gray-400">السعر</p><p className="text-xs font-bold text-gray-900">{card.price} ر.ي</p></div>
                </div>
